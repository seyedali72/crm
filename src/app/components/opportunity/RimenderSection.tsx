...
export default function ReminderSectionOppo({ singleOpportunity, mutated, owner }: any) {
    ...
    const fetchData = useCallback(async () => {
        if (singleOpportunity?._id !== undefined) {
            let data = await getReminders({ opportunityId: singleOpportunity?._id, isDeleted: false })
            setReminders(data)
        }
    }, [singleOpportunity])
    const handleCreateReminder = async (obj: any) => {
        ...
        obj.leadId = singleOpportunity?.leadId?._id
        obj.opportunityId = singleOpportunity?._id
