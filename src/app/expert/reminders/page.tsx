...
export default function RemindersList() {
    ...
    const fetchReminders = useCallback(async () => {
        if (user?._id !== undefined) {
            let list = await getReminders({ isDeleted: false, expertId: user?._id })
            ...
            {popup && <div className="popupCustom">
                <section className="main-body-container rounded">
                    ...
                    <form onSubmit={handleSubmit(handleEditReminder)} method="post" className="col-12 d-flex flex-column gap-2">
                        ...
                        <div className="col-12"><button type="submit" className="btn w-100 btn-sm bg-custom-4 text-white px-3" >ثبت ویرایش</button></div>
                    </form>
                    <div className="col-12 d-flex gap-2"><button type="button" onClick={() => toast(<Confirmation type='تکمیل' onDelete={() => complateReminder()} />, { autoClose: false, })} className="btn mt-2 w-100 btn-sm bg-custom-2 text-white px-3" >تکمیل یادآور</button>
                                ...
                                if (val !== undefined && Reminder?.type.includes(val)) {
                                    return (<tr key={idx}>
                                        ...
                                        <td className="text-center">
                                            ...
                                            <button type="button" className="btn btn-sm bg-custom-3 ms-1" onClick={() => toast(<Confirmation onDelete={() => handleDelete(Reminder?._id)} />, { autoClose: false, })}>
