...
export default function CustomersList() {
    ...
    const fetchCustomers = useCallback(async () => {
        if (user?._id !== undefined) {
            let list = await getCustomers({ isDeleted: false, expert: user._id })
                                ...
                                if (customer?.contactId?.name.includes(filter) || customer?.contactId?.phone_number_1.includes(filter)) {
                                    return (<tr key={idx}>
                                        ...
                                        <td className="text-center">
                                            <Link href={`/expert/customers/${customer?._id}`} className="btn btn-sm bg-custom-4 ms-1" ><i className="fa fa-edit px-1"></i>پرونده</Link>
