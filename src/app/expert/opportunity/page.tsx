...
export default function OpportunitiesList() {
    ...
    const fetchOpportunities = useCallback(async () => {
        if (user?._id !== undefined) {
            let list = await getOpportunities({ isDeleted: false, expert: user?._id })
                                ...
                                if (opportunity?.title.includes(filter)) {
                                    return (<tr key={idx}>
                                        ...
                                        <td className="text-center">
                                            <Link href={`/expert/opportunity/${opportunity?._id}`} className="btn btn-sm bg-custom-4 ms-1" ><i className="fa fa-edit px-1"></i>پرونده</Link>
