...
export default function Home() {
  ...
  const convertToLead = async (obj: any) => {
    ...
    if (!res?.error) {
      ...
      router.replace(`/expert/leads/${res?._id}`)
                  ...
                  return (<tr key={idx}>
                    ...
                    {company?.converted ?
                      <td><button type="button" className="btn btn-sm bg-custom-2 ms-1" onClick={() => checkStatus(company?._id)}> <i className="fa fa-refresh px-1"></i>جزئیات </button>
                        <Link href={`/expert/companies/${company?._id}`} className="btn btn-sm bg-custom-4 ms-1" ><i className="fa fa-edit px-1"></i> ویرایش</Link></td>
                      : <td className="text-center">
                        ...
                        <Link href={`/expert/companies/${company?._id}`} className="btn btn-sm bg-custom-4 ms-1" ><i className="fa fa-edit px-1"></i> ویرایش</Link>
